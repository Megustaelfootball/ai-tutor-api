...
// Restricción de CORS la dejaremos "dinamica" para que acepte Vercel sin importar qué URL nos den hoy
app.use(cors({
    origin: function(origin, callback){
        // Aceptamos llamadas locales y llamadas desde Vercel
...
app.post('/api/checkout', async (req, res) => {
    ...
    try {
        const session = await stripe.checkout.sessions.create({
            ...
            // Usamos req.headers.origin para que Stripe sepa a dónde regresar al usuario, sea Localhost o Vercel
            success_url: `${req.headers.origin}/success?session_id={CHECKOUT_SESSION_ID}&task_id=${taskId}`,
            cancel_url: `${req.headers.origin}/canceled?task_id=${taskId}`,
        });
